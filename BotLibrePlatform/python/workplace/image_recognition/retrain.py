...
def add_final_training_ops_B(class_count, final_tensor_name, bottleneck_tensor,
                           bottleneck_tensor_size):
  """Adds a new softmax and fully-connected layer for training.
...
def save_graph_to_file_B(sess, graph, graph_file_name):
  output_graph_def = graph_util.convert_variables_to_constants(
...
def prepare_system(summaries_dir):
...
def start_training(architecture,image_dir,model_dir, output_graph, bottleneck_dir,summaries_dir,how_many_training_steps):
  # Needed to make sure the logging output is visible.
  # See https://github.com/tensorflow/tensorflow/issues/3047
  tf.logging.set_verbosity(tf.logging.INFO)

  # Prepare necessary directories  that can be used during training
  prepare_system(summaries_dir)
  ...
  # Look at the folder structure, and create lists of all the images.
  image_lists = create_image_lists(image_dir, 10,
                                  10)
  class_count = len(image_lists.keys())
  if class_count == 0:
  ...
  with tf.Session(graph=graph) as sess:
    ...
    # Add the new layer that we'll be training.
    (train_step, cross_entropy, bottleneck_input, ground_truth_input,
    final_tensor) = add_final_training_ops_B(
        len(image_lists.keys()), "final_result", bottleneck_tensor,
        model_info['bottleneck_tensor_size'])
    ...
    for i in range(how_many_training_steps):
      ...
      if (intermediate_frequency > 0 and (i % intermediate_frequency == 0)
          and i > 0):
        intermediate_file_name = (bottleneck_dir + "/tmp/intermediate_graph" +
                                  'intermediate_' + str(i) + '.pb')
        tf.logging.info('Save intermediate result to : ' +
                        intermediate_file_name)
        save_graph_to_file_B(sess, graph, intermediate_file_name)
    ...
    # Write out the trained graph and labels with the weights stored as
    # constants.
    save_graph_to_file_B(sess, graph, output_graph+"retrained_graph.pb")
    with gfile.FastGFile(output_graph+"retrained_labels.txt", 'w') as f:
      f.write('\n'.join(image_lists.keys()) + '\n')