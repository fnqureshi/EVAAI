...
def main(_):
  assert "training/", '`train_dir` is missing.'
  tf.gfile.MakeDirs("training/")
  if "training/faster_rcnn_inception_v2_pets.config":
    configs = config_util.get_configs_from_pipeline_file(
        "training/faster_rcnn_inception_v2_pets.config")
    
    tf.gfile.Copy("training/faster_rcnn_inception_v2_pets.config",
                  os.path.join("training/", 'pipeline.config'),
                  overwrite=True)
  else:
    ...
    for name, config in [('model.config', ""),
                          ('train.config', ""),
                          ('input.config', "")]:
      tf.gfile.Copy(config, os.path.join("training/", name),
                    overwrite=True)
