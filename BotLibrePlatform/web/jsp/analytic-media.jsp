	...
	<div id="admin-topper" align="left">
		<div class="clearfix">
			<span class="dropt-banner">
				...
				<div>
					<%= loginBean.translate("The analytics media page allows you to upload images and media.") %><br/>
					<%= loginBean.translate("You can label the images with a label that classifies the image.") %><br/>
					<%= loginBean.translate("This data can then be used to train a network for image classification.") %><br/>
				</div>
...
<% } %>	
	...
	<% } else { %>
		<h1>
			...
			</span> <%= loginBean.translate("Media Repository") %>
		</h1>
		<form id="analyticForm" action="analytic" method="post" class="message">
			...
			<script type="text/javascript">
				...
				function deleteSelectedMedia() {
					...
					for (var i = 0; i < divs.length; i++) {
						...
						if (checkbox != null && checkbox.checked) {
							...
							connectSDK.deleteAnalyticMedia(analyticMediaConfig, function() {
				...
				function createAnalyticLabel(){
					...
					connectSDK.createAnalyticLabel(labelConfig);
				...
				function deleteAnalyticLabel() {
					...
					connectSDK.deleteAnalyticLabel(labelConfig);
					select.remove(select.selectedIndex);
					//updating media
					getAnalyticMedia();
				}
				
				function getAnalyticMedia() {
					...
					//passing AnalyticId
					connectSDK.getAnalyticMedia(analyticConfig, function(medias) {
						try {
				...
				window.onload = function () { getAnalyticMedia(); }
			...
			<select id="labels" name="selected-label" title="Labels" onChange="getAnalyticMedia();">
				<% List<AnalyticLabel> labels = bean.getInstance().getLabels(); %>
					<% if (!labels.isEmpty()) { %>
						<% for (AnalyticLabel label : labels) { %>
							<option value="<%= label.getLabel() %>" <%= bean.isLabelSelected(label.getLabel()) %>><%= label.getLabel() %></option>
						<% } %>
					<% } %>	
			</select>
			<hr/>
