								...
								<% if (!Site.COMMERCIAL || (Site.DEDICATED && !Site.CLOUD)) { %>
									<% if (!loginBean.getViewUser().getPayments().isEmpty()) { %>
										...
										<% for (UserPayment payment : loginBean.getViewUser().getPayments()) { %>
											<br/>
											<%= payment.getPaymentDate() %> : <%= payment.getType() %> : <%= payment.getPaymentDuration() %> : <%= payment.getStatus() %> : <%= payment.getPaypalAmt() %> - <%= payment.getPaypalTx() %> - <%= payment.getPaypalCc() %> - <%= payment.getPaypalSt() %> - <%= payment.getUserId() %>
										<% } %>
									<% } %>
								<% } %>
							<% } %>
