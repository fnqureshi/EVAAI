...
<head>
	...
	<title><%= loginBean.translate("Search Analytics") %> - <%= Site.NAME %></title>
	<meta name="description" content="<%= loginBean.translate("Search the analytic directory") %>"/>
	<meta name="keywords" content="<%= loginBean.translate("search, directory, analytics, files") %>"/>
			...
			<div class="browse">
				...
				<% if (allowAccess) { %>
					<h3 onclick="document.getElementById('search').click()" class="clickable"><img src="images/search.svg" class="admin-banner-pic" style="width:28px;padding:4px;"/>
						<%= loginBean.translate("Search Analytics") %>
					</h3>
					<%= bean.searchFormHTML() %>
					<%= bean.searchHTML() %>
				<% } %>
