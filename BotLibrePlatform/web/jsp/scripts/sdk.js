	...
	if (playPromise !== undefined && SDK.canPlayAudio == null) {
		playPromise.then(function() {
			...
			if (SDK.canPlayAudio == null) {
				...
				baseRoot.appendChild(playButton);
				setTimeout(function() {
 ...
 */
SDK.tts = function(text, voice, native, lang, nativeVoice, mod, apiKey, apiEndpoint) {
	try {
 ...
 */
SDK.bingSpeechTTS = function(utterance, lang, voice, apiKey, apiEndpoint) {
 ...
 */
SDK.nativeTTS = function(utterance, lang, voice, apiKey, apiEndpoint) {
...
function WebLiveChatListener() {
	...
	this.createBox = function() {
		...
		html = html
			...
			if (this.showMenubar) {
				...
				html = html
					...
					+ "</table>\n"
					+ "</div>\n"
					+ "<img class='" + this.prefix + "toolbar' src='" + SDK.url + "/images/menu.svg'>\n"
					+ "</span>\n"
					+ "</div>\n"
					+ "</span></td>\n";
			}
		...
		if (this.baseRoot.getElementById(this.prefix + "toggleChime") != null) {
			this.baseRoot.getElementById(this.prefix + "toggleChime").addEventListener("click", function() {
				self.toggleChime();
				if (self.playChime) {
					self.baseRoot.getElementById('boxchime').src = SDK.url + "/images/sound.svg";
				} else {
					self.baseRoot.getElementById('boxchime').src = SDK.url + "/images/mute.svg";
				}
			});
		}
		if (this.baseRoot.getElementById(this.prefix + "toggleSpeak") != null) {
			this.baseRoot.getElementById(this.prefix + "toggleSpeak").addEventListener("click", function() {
				self.toggleSpeak();
				if (self.speak) {
					self.baseRoot.getElementById('boxtalk').src = SDK.url + "/images/talk.svg";
				} else {
					self.baseRoot.getElementById('boxtalk').src = SDK.url + "/images/talkoff.svg";
				}
				return false;
			});
		}
	...
	this.updateUsersXML = function(usersXML) {
		...
		if (this.onlineBar) {
			...
			if (this.chatroom || this.isFrame) { // displaying list of users on top
				...
				if (!this.isFrame) {
					...
					var line = this.baseRoot.createElement('hr');
		...
		var div = this.baseRoot.createElement('div');
		div.innerHTML = usersXML;
		var children = div.childNodes[0].childNodes;
		var count = 0;
		var length = children.length;
