		...
		try {
			...
			if (next != null) {
				if (bean.confirmPayment("", "", "", "", "", "", "", "")) {
					...
					AdminDatabase.instance().log(Level.INFO, "payment error");
			...
			if (ipn_track_id != null) {
				...
				AdminDatabase.instance().log(Level.INFO, "######### TransactionType : "+transactionType+"  ======== IPN verified : "
					...
					public void run() {
						try {
							...
							if (verifyIPN.equalsIgnoreCase("VERIFIED")) {
								AdminDatabase.instance().log(Level.INFO, "Verified IPN: IPN message for Payment ID: " + payment_id + 
										(transactionType.equalsIgnoreCase("subscr_payment") ? " and transation-id: "+ transaction_id + " is verified." : ""));
							} else {
								AdminDatabase.instance().log(Level.INFO, "Invalid IPN: IPN message for Payment ID: " + payment_id +
			...
			if (tx != null) {
				AdminDatabase.instance().log(Level.INFO, "processing payment...");
				if (!bean.confirmPayment(tx, amt, st, cc, custom, payment_id, payment_date, token)) {
					AdminDatabase.instance().log(Level.INFO, "invalid payment: unable to process");
					response.sendRedirect("create-domain.jsp");
				} else {
					AdminDatabase.instance().log(Level.INFO, "valid payment: processing complete");
			...
			if (tkn != null) {
				AdminDatabase.instance().log(Level.INFO, "recieved tkn: " + tkn);
				response.sendRedirect("upgrade-complete.jsp");
				response.setStatus(200);
				return;
			}
