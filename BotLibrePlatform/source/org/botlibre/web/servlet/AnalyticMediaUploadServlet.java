 ...
 */
@javax.servlet.annotation.WebServlet("/analytic-media-upload")
@SuppressWarnings("serial")
@MultipartConfig
public class AnalyticMediaUploadServlet extends BeanServlet {
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if(selectedLabel !=null){
				...
				bean.setSelectedLabel(selectedLabel);
			}
			...
			if (!bean.isSuper() && !bean.getUser().isPartnerUser()) {
				...
				availableSlots = allowedImages - bean.getInstance().getMedia().size();
			...
			for (Part filePart : files) {
				...
				bean.createAnalyticMedia(image, fileName, filePart.getContentType(), selectedLabel);
		...
		response.sendRedirect("analytic-media.jsp");
	}
}
