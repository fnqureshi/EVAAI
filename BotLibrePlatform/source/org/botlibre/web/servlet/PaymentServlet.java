	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if (paymentType != null) {
				...
				AdminDatabase.instance().log(Level.INFO, "payment information received: "+paymentType+" "+userType+" "+duration);
				if (cancel != null) {
			...
			if (complete != null) {
				if (bean.getUser().isSuperUser()) {
					...
					AdminDatabase.instance().log(Level.INFO, "payment processed: Complete");
			...
			if (checkPayment != null) {
				AdminDatabase.instance().log(Level.INFO, "checking payment status");
			...
			if (isIPN) {
				...
				AdminDatabase.instance().log(Level.INFO, "IPN: " + url);
			}
			...
			if (tkn != null) {
				AdminDatabase.instance().log(Level.INFO, "recieved tkn for subscription: " + tkn);
				response.sendRedirect("upgrade-complete.jsp");
			...
			if (ipn_track_id != null) {
				...
				AdminDatabase.instance().log(Level.INFO, "IPN : "+ map + " "
					...
					public void run() {
						try {
							...
							if (verifyIPN.equalsIgnoreCase("VERIFIED")) {
								AdminDatabase.instance().log(Level.INFO, "IPN: Verified IPN message for Transaction ID: " + transaction_id + " is verified.");
							} else {
								AdminDatabase.instance().log(Level.INFO, "IPN: Invalid IPN message for Transaction ID: " + transaction_id + " is invalid.");
							}
			...
			if (tx != null) {
				AdminDatabase.instance().log(Level.INFO, "recieved payment, processing : tx: "+tx+
						" amt: "+amt+" st: "+st+" cc: "+cc+" payment_id: "+custom);
				response.setStatus(200);
				bean.confirmPayment(tx, amt, st, cc, custom, sub_date, isIPN, token);
				response.sendRedirect("upgrade-complete.jsp");
				if (!isIPN) {
					return;
				}
