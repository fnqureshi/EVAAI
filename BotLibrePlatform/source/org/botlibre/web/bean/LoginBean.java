...
public class LoginBean extends ServletBean {
	...
	public Friendship createUserFriendship(String userTargetId) {
		try {
			...
			if (botUser != null) {
				userTargetId = botUser.getUserId();
			}
	...
	public boolean updateCategory(String name, String description, String parents, boolean isSecure) {
		try {
			...
			} else if (category.getType().equals("Analytic")) {
				getBean(AnalyticBean.class).setCategory(getCategory());
	...
	public boolean deleteCategory() {
		...
		try {
			...
			if (category.getType().equals("Bot")) {
				...
				getBean(AnalyticBean.class).setCategory(null);
			} else if (category.getType().equals("Forum")) {
	...
	public boolean updateCategory(byte[] image) {
		try {
			...
			if (this.category.getType().equals("Bot")) {
				getBotBean().setCategory(getCategory());
			} else if (this.category.getType().equals("Analytic")) {
				getBean(AnalyticBean.class).setCategory(getCategory());
	...
	public void confirmPayment(String tx, String amt, String st, String cc, String id, String date, boolean IPN, String token) {
		AdminDatabase.instance().log(Level.INFO, "confirm payment " + "tx: "+ tx + " amt: " + amt + " st: " + st + " cc: " + cc + " id: " + id);
		try {
			...
			if (IPN) {
				...
				if ((!(initialDate.substring(4, 10) + ", " + initialDate.substring(24, 28)).equalsIgnoreCase(date.substring(9, 21))) 
					...
					if (uniqueTransaction) {
						AdminDatabase.instance().log(Level.INFO, "processing recurring subscription payment: tx " + tx);
						beginPayment(initialPayment.getUserType(), initialPayment.getType(), "recurring", "", "", "", String.valueOf(initialPayment.getPaymentDuration()), initialPayment.isSubscription());
					} else {
						AdminDatabase.instance().log(Level.INFO, "transaction "+ tx +" already processed");
						return;
					}
				}
			}
			if (this.payment.getStatus().equals(UserPaymentStatus.Complete)) {
				AdminDatabase.instance().log(Level.INFO, "payment already processed");
				return;
			}
			...
			if (!this.payment.getUserType().equals(UserType.Avatar)) {
				...
				AdminDatabase.instance().log(Level.INFO, "updated user "+this.user.getUserId()+" with "+this.payment.getUserType());
			}
			this.payment = null;
		} catch (Exception exception) {
			error(exception);
