...
import org.botlibre.web.analytic.AnalyticConfigTest;
import org.botlibre.web.analytic.AnalyticMedia;
import org.botlibre.web.analytic.AnalyticMediaConfig;
import org.botlibre.web.analytic.AnalyticTestMedia;
import org.botlibre.web.analytic.AnalyticTrainingData;
import org.botlibre.web.analytic.LabelConfig;
...
public class BOTlibreRestServer {
	...
	public List<WebMediumConfig> getInstances(BrowseConfig config, @Context HttpServletRequest requestContext) {
		try {
			...
			} else if (config.type.equals("Analytic")) {
				bean = loginBean.getBean(AnalyticBean.class);
			} else {
				bean = loginBean.getBotBean();
			}
		...
		try {
			...
			if ("AddFriendship".equals(config.action)) {
				...
				if (friendship != null) {
					...
					userConfig = new UserConfig(user, false);
	...
	public ChannelConfig getUserChannel(UserConfig config, @Context HttpServletRequest requestContext) {
		...
		try {
			...
			if (botInstance == null) {
				...
				BotBean botBean = loginBean.getBotBean();
				boolean createInstance = botBean.createInstance(instanceConfig, false, false, extractIP(requestContext));
				if (createInstance) {
	...
	@Produces(MediaType.APPLICATION_XML)
	@Path("/get-analytics")
	public List<AnalyticConfig> getAnalytics(BrowseConfig config, @Context HttpServletRequest requestContext) {
		AdminDatabase.instance().log(Level.INFO, "API get-analytics");
	...
	@Consumes(MediaType.APPLICATION_XML)
	@Produces(MediaType.APPLICATION_XML)
	@Path("/get-analytic-users")
	public List<UserConfig> getAnalyticUsers(AnalyticConfig config, @Context HttpServletRequest requestContext) {
		AdminDatabase.instance().log(Level.INFO, "API get-analytic-users");
		return getUsers(config, requestContext);
	}
	...
	@Consumes(MediaType.APPLICATION_XML)
	@Produces(MediaType.APPLICATION_XML)
	@Path("/get-analytic-admins")
	public List<UserConfig> getAnalyticAdmins(AnalyticConfig config, @Context HttpServletRequest requestContext) {
		AdminDatabase.instance().log(Level.INFO, "API get-analytic-admins");
		return getAdmins(config, requestContext);
	}
	...
	@Produces(MediaType.APPLICATION_XML)
	@Path("/test-analytic")
	public String testAnalytic(
			@FormDataParam("file") InputStream stream,
			@FormDataParam("xml") AnalyticConfig config, @Context HttpServletRequest requestContext) {
		AdminDatabase.instance().log(Level.INFO, "API test-analytic");
		try {
			if (stream == null) {
				throw new BotException("Missing file attachment");
			}
			...
			LoginBean loginBean = new LoginBean();
			try {
				AnalyticBean bean = (AnalyticBean)config.validate(loginBean, requestContext);
				byte[] image = BotBean.loadImageFile(stream);
				
				String response = bean.testAnalytic(image,  bean.getInstance());
	...
	@Consumes(MediaType.MULTIPART_FORM_DATA)
	@Produces(MediaType.APPLICATION_XML)
	@Path("/test-audio-analytic")
	public String testAudioAnalytic(
			@FormDataParam("file") InputStream stream,
			@FormDataParam("xml") AnalyticConfig config, @Context HttpServletRequest requestContext) {
		AdminDatabase.instance().log(Level.INFO, "API test-audio-analytic");
		try {
			if (stream == null) {
				throw new BotException("Missing file attachment");
			}
			...
			try {
				AnalyticBean bean = (AnalyticBean)config.validate(loginBean, requestContext);
				byte[] audio = BotBean.loadImageFile(stream);
				
				String response = bean.testAudioAnalytic(audio,  bean.getInstance());
	...
	@Consumes(MediaType.MULTIPART_FORM_DATA)
	@Produces(MediaType.APPLICATION_XML)
	@Path("/test-object-detection-analytic")
	public String testObjectDetectionAnalytic(
			@FormDataParam("file") InputStream stream,
			@FormDataParam("xml") AnalyticConfigTest config, @Context HttpServletRequest requestContext) {
		AdminDatabase.instance().log(Level.INFO, "API test-object-detection-analytic");
		try {
			if (stream == null) {
				throw new BotException("Missing file attachment");
			}
			...
			try {
				...
				String response = bean.testObjectDetectionAnalytic(image,  bean.getInstance());
	...
	@Consumes(MediaType.APPLICATION_XML)
	@Produces(MediaType.APPLICATION_XML)
	@Path("/get-test-media-result")
	public String getTestMediaResult(
			AnalyticConfig config, @Context HttpServletRequest requestContext) {
		AdminDatabase.instance().log(Level.INFO, "API get-test-media-result");
		try {
			...
			try {
				...
				bean.checkInstance();
				bean.checkAdmin();
				String response = bean.getInstance().getTestMediaResult();
	...
	@Consumes(MediaType.APPLICATION_XML)
	@Produces(MediaType.APPLICATION_XML)
	@Path("/report-media-analytic")
	public AnalyticConfig reportMediaAnalytic(
			AnalyticConfig config, @Context HttpServletRequest requestContext) {
		AdminDatabase.instance().log(Level.INFO, "API report-media-analytic");
		try {
			LoginBean loginBean = new LoginBean();
			try {
				...
				if (config.processingTestMediaStatus == null) {
					config.processingTestMediaStatus = "started";
				}
				bean.reportMediaAnalytic(config.processingTestMediaStatus);
				bean.refreshEntity();
				WebMedium instance = (WebMedium)bean.getInstance();
				config = (AnalyticConfig) instance.buildConfig();
	...
	@Produces(MediaType.APPLICATION_XML)
	@Path("/train-analytic")
	public AnalyticConfig trainAnalytic(
			AnalyticConfig config, @Context HttpServletRequest requestContext) {
		AdminDatabase.instance().log(Level.INFO, "API train-analytic");
		try {
			LoginBean loginBean = new LoginBean();
			try {
				AnalyticBean bean = (AnalyticBean)config.validate(loginBean, requestContext);
				...
				//avoid null pointer exception
				if (config.trainingStatus == null){
					config.trainingStatus = "started";
				}
				bean.trainAnalytic(config.trainingStatus);
				bean.refreshEntity();
				WebMedium instance = (WebMedium)bean.getInstance();
				config = (AnalyticConfig) instance.buildConfig();
	...
	@Consumes(MediaType.APPLICATION_XML)
	@Produces(MediaType.APPLICATION_XML)
	@Path("/delete-analytic-media")
	public AnalyticMediaConfig deleteAnalyticMedia(
			AnalyticMediaConfig config, @Context HttpServletRequest requestContext) {
		AdminDatabase.instance().log(Level.INFO, "API delete-analytic-media");
		try {
			LoginBean loginBean = new LoginBean();
			try {
				AnalyticBean bean = (AnalyticBean)config.validate(loginBean, requestContext);
				
				bean.deleteAnalyticMedia(Long.parseLong(config.mediaId));
				bean.setSelectedLabel(config.label);
				if (loginBean.getError() != null) {
					throw loginBean.getError();
				}
				return config;
	...
	@Consumes(MediaType.APPLICATION_XML)
	@Produces(MediaType.APPLICATION_XML)
	@Path("/delete-analytic-test-media")
	public AnalyticMediaConfig deleteAnalyticTestMedia(
			AnalyticMediaConfig config, @Context HttpServletRequest requestContext) {
		AdminDatabase.instance().log(Level.INFO, "API delete-analytic-test-media");
		try {
			LoginBean loginBean = new LoginBean();
			try {
				AnalyticBean bean = (AnalyticBean)config.validate(loginBean, requestContext);
				
				bean.deleteAnalyticTestMedia(Long.parseLong(config.mediaId));
				bean.setSelectedTestMediaLabel(config.label);
				if (loginBean.getError() != null) {
					throw loginBean.getError();
				}
				return config;
	...
	@Consumes(MediaType.APPLICATION_XML)
	@Produces(MediaType.APPLICATION_XML)
	@Path("/get-analytic-media")
	public List<AnalyticMediaConfig> getAnalyticMedia(AnalyticConfig config, @Context HttpServletRequest requestContext) {
		AdminDatabase.instance().log(Level.INFO, "API get-analytic-media");
		try {
			LoginBean loginBean = new LoginBean();
			AnalyticBean bean = (AnalyticBean)config.validate(loginBean, requestContext);
			bean.checkAdmin();
			List<AnalyticMedia> instances =  bean.getInstance().getMedia();
			List<AnalyticMediaConfig> configs = new ArrayList<AnalyticMediaConfig>();
			for (AnalyticMedia instance : instances) {
				AnalyticMediaConfig mediaConfig = instance.toConfig();
				configs.add(mediaConfig);
			}
			return configs;
		} catch (Throwable failed) {
			error(failed);
	...
	@Consumes(MediaType.APPLICATION_XML)
	@Produces(MediaType.APPLICATION_XML)
	@Path("/get-analytic-test-media")
	public List<AnalyticMediaConfig> getAnalyticTestMedia(AnalyticConfig config, @Context HttpServletRequest requestContext) {
		AdminDatabase.instance().log(Level.INFO, "API get-analytic-test-media");
		try {
			LoginBean loginBean = new LoginBean();
			AnalyticBean bean = (AnalyticBean)config.validate(loginBean, requestContext);
			bean.checkAdmin();
			List<AnalyticTestMedia> instances =  bean.getInstance().getTestMedia();
			List<AnalyticMediaConfig> configs = new ArrayList<AnalyticMediaConfig>();
			for (AnalyticTestMedia instance : instances) {
				AnalyticMediaConfig mediaConfig = instance.toConfig();
				configs.add(mediaConfig);
			}
			return configs;
	...
	@Consumes(MediaType.APPLICATION_XML)
	@Produces(MediaType.APPLICATION_XML)
	@Path("/create-analytic-label")
	public LabelConfig createAnalyticLabel(
			LabelConfig config, @Context HttpServletRequest requestContext) {
		AdminDatabase.instance().log(Level.INFO, "API create-analytic-label");
		try {
			LoginBean loginBean = new LoginBean();
			try {
				AnalyticBean bean = (AnalyticBean)config.validate(loginBean, requestContext);
				
				bean.createAnalyticLabel(config.label);
				if (loginBean.getError() != null) {
					throw loginBean.getError();
				}
				return config;
	...
	@Consumes(MediaType.APPLICATION_XML)
	@Produces(MediaType.APPLICATION_XML)
	@Path("/create-analytic-test-media-label")
	public LabelConfig createAnalyticTestMediaLabel(
			LabelConfig config, @Context HttpServletRequest requestContext) {
		AdminDatabase.instance().log(Level.INFO, "API create-analytic-test-media-label");
		try {
			LoginBean loginBean = new LoginBean();
			try {
				AnalyticBean bean = (AnalyticBean)config.validate(loginBean, requestContext);
				
				bean.createAnalyticTestMediaLabel(config.label);
				if (loginBean.getError() != null) {
					throw loginBean.getError();
				}
				return config;
	...
	@Consumes(MediaType.APPLICATION_XML)
	@Produces(MediaType.APPLICATION_XML)
	@Path("/delete-analytic-label")
	public LabelConfig deleteAnalyticLabel(
			LabelConfig config, @Context HttpServletRequest requestContext) {
		AdminDatabase.instance().log(Level.INFO, "API delete-analytic-label");
		try {
			LoginBean loginBean = new LoginBean();
			try {
				AnalyticBean bean = (AnalyticBean)config.validate(loginBean, requestContext);
				
				bean.deleteAnalyticLabel(config.label);
				if (loginBean.getError() != null) {
					throw loginBean.getError();
				}
				return config;
	...
	@Consumes(MediaType.APPLICATION_XML)
	@Produces(MediaType.APPLICATION_XML)
	@Path("/delete-analytic-test-media-label")
	public LabelConfig deleteAnalytiTestMediacLabel(
			LabelConfig config, @Context HttpServletRequest requestContext) {
		AdminDatabase.instance().log(Level.INFO, "API delete-analytic-test-media-label");
		try {
			LoginBean loginBean = new LoginBean();
			try {
				AnalyticBean bean = (AnalyticBean)config.validate(loginBean, requestContext);
				
				bean.deleteAnalyticTestMediaLabel(config.label);
				if (loginBean.getError() != null) {
					throw loginBean.getError();
				}
				return config;
	...
	@Produces(MediaType.APPLICATION_XML)
	@Path("check-training")
	public AnalyticConfig refreshAnalyticTrainingStatus(AnalyticConfig config, @Context HttpServletRequest requestContext) {
		AdminDatabase.instance().log(Level.INFO, "API check-training");
		try {
			LoginBean loginBean = new LoginBean();
			try {
				AnalyticBean bean = (AnalyticBean)config.validate(loginBean, requestContext);
	...
	@Consumes(MediaType.APPLICATION_XML)
	@Produces(MediaType.APPLICATION_XML)
	@Path("create-analytic")
	public AnalyticConfig createAnalytic(AnalyticConfig config, @Context HttpServletRequest requestContext) {
		AdminDatabase.instance().log(Level.INFO, "API create-analytic", config.name);
		try {
			LoginBean loginBean = new LoginBean();
			config.connect(loginBean, requestContext);
			AnalyticBean bean = loginBean.getBean(AnalyticBean.class);
			...
			Analytic instance = bean.getInstance();
			config = instance.buildConfig();
			config.isAdmin = true;
			config.avatar = bean.getAvatarImage(bean.getInstance());
			return config;
	...
	public AnalyticConfig updateAnalytic(AnalyticConfig config, @Context HttpServletRequest requestContext) {
		AdminDatabase.instance().log(Level.INFO, "API update-script", config.name);
		try {
			LoginBean loginBean = new LoginBean();
			try {
				AnalyticBean bean = (AnalyticBean)config.validate(loginBean, requestContext);
				bean.updateAnalytic(config, config.domain, null, null);
				if (loginBean.getError() != null) {
					throw loginBean.getError();
				}
				Analytic instance = bean.getInstance();
	...
	@Produces(MediaType.TEXT_PLAIN)
	@Path("flag-analytic")
	public void flagAnalytic(AnalyticConfig config, @Context HttpServletRequest requestContext) {
		AdminDatabase.instance().log(Level.INFO, "API flag-analytic");
		flag(config, requestContext);
	}
	...
	@Consumes(MediaType.APPLICATION_XML)
	@Produces(MediaType.TEXT_PLAIN)
	@Path("thumbs-up-analytic")
	public void thumbsUpAnalytic(AnalyticConfig config, @Context HttpServletRequest requestContext) {
		AdminDatabase.instance().log(Level.INFO, "API thumbs-up-analytic");
		thumbsUp(config, requestContext);
	}
	...
	@Consumes(MediaType.APPLICATION_XML)
	@Produces(MediaType.TEXT_PLAIN)
	@Path("thumbs-down-analytic")
	public void thumbsDownAnalytic(AnalyticConfig config, @Context HttpServletRequest requestContext) {
		AdminDatabase.instance().log(Level.INFO, "API thumbs-down-analytic");
		thumbsDown(config, requestContext);
	}
	...
	@Produces(MediaType.TEXT_PLAIN)
	@Path("star-analytic")
	public void starAnalytic(AnalyticConfig config, @Context HttpServletRequest requestContext) {
		AdminDatabase.instance().log(Level.INFO, "API star-analytic");
		star(config, requestContext);
	}
	...
	@Consumes(MediaType.APPLICATION_XML)
	@Produces(MediaType.TEXT_PLAIN)
	@Path("delete-analytic")
	public void deleteAnalytic(AnalyticConfig config, @Context HttpServletRequest requestContext) {
		AdminDatabase.instance().log(Level.INFO, "API delete-analytic");
		delete(config, requestContext);
	}
	...
	@Produces(MediaType.APPLICATION_XML)
	@Path("check-analytic")
	public AnalyticConfig checkAnalytic(AnalyticConfig config, @Context HttpServletRequest requestContext) {
		AdminDatabase.instance().log(Level.INFO, "API check-analytic", config.name);
		return (AnalyticConfig)check(config, requestContext);
	}
	
	@POST
	@Consumes(MediaType.APPLICATION_XML)
	@Produces(MediaType.APPLICATION_XML)
	@Path("/reset-data-analytic")
	public void resetDataAnalytic(
			AnalyticConfig config, @Context HttpServletRequest requestContext) {
		AdminDatabase.instance().log(Level.INFO, "API reset-data-analytic");
		try {
			LoginBean loginBean = new LoginBean();
			try {
				AnalyticBean bean = (AnalyticBean)config.validate(loginBean, requestContext);
	...
	@Consumes(MediaType.APPLICATION_XML)
	@Produces(MediaType.APPLICATION_XML)
	@Path("/test-data-analytic")
	public String testDataAnalytic(
			AnalyticConfig config, @Context HttpServletRequest requestContext) {
		AdminDatabase.instance().log(Level.INFO, "API test-data-analytic");
		try {
			LoginBean loginBean = new LoginBean();
			try {
				AnalyticBean bean = (AnalyticBean)config.validate(loginBean, requestContext);
				
				String response = bean.testDataAnalytic(config, bean.getInstance());
	...
	@Consumes(MediaType.APPLICATION_XML)
	@Produces(MediaType.APPLICATION_XML)
	@Path("/train-data-analytic")
	public String trainDataAnalytic(
			AnalyticTrainingData config, @Context HttpServletRequest requestContext) {
		AdminDatabase.instance().log(Level.INFO, "API train-data-analytic");
		try {
			LoginBean loginBean = new LoginBean();
			try {
				AnalyticBean bean = (AnalyticBean)config.validate(loginBean, requestContext);
				
				String response = bean.trainDataAnalytic(config, bean.getInstance());
