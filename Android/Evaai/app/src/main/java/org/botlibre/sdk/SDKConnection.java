...
public class SDKConnection {
	...
	public UserConfig verifyEmail(UserConfig config) {
		config.addCredentials(this);
		String xml = POST(this.url + "/verify-email", config.toXML());
		Element root = parse(xml);
		if (root == null) {
			this.user = null;
			return null;
	...
	public UserConfig deleteUserAccount(UserConfig config) {
		config.addCredentials(this);
		String xml = POST(this.url + "/delete-user-account", config.toXML());
	...
	public URL fetchImage(String image) {
		try {
			if (image.startsWith("http:") || image.startsWith("https:")) {
				...
				return new URL("https://" + this.credentials.host + this.credentials.app + "/" + image);
			}
		} catch (Exception exception) {
	 ...
	 */
	public UserMessageConfig createUserMessage(UserMessageConfig config) {
		config.addCredentials(this);
		UserMessageConfig message = null;
		String xml = POST(this.url + "/create-user-message", config.toXML());
		if (xml == null || xml.equals("")) return message;
		message = new UserMessageConfig();
		Element root = parse(xml);
		message.parseXML(root);
		return message;
	}
	...
	/**
	 * Fetch the channel content details from the server.
	 * The id or name and domain of the object must be set.
	 */
	@SuppressWarnings("unchecked")
	public ChannelConfig fetchChannel(Config config) {
		String xml;
		config.addCredentials(this);
		if(config instanceof InstanceConfig) {
			xml = POST(this.url + "/get-bot-channel", config.toXML());
		} else if (config instanceof UserConfig) {
			xml = POST(this.url + "/get-user-channel", config.toXML());
		} else {
			return null;
		}
		Element root = parse(xml);
	...
	/**
	 * Process the user's avatar message and return the avatars response.
	 * This allows the speech and video animation for a user's avatar to be generated for the message.
	 */
	public ChatResponse userAvatarMessage(UserAvatarMessage config) {
		config.addCredentials(this);
		String xml = POST(this.url + "/user-avatar-message", config.toXML());
		Element root = parse(xml);
	...
	/**
	 * Save the user's voice configuration.
	 */
	public void saveUserVoice(VoiceConfig config) {
		config.addCredentials(this);
		POST(this.url + "/save-user-voice", config.toXML());
	}
	...
	/**
	 * Save the user's avatar configuration.
	 */
	public void saveUserAvatar(InstanceConfig config) {
		config.addCredentials(this);
		POST(this.url + "/save-user-avatar", config.toXML());
	}
	...
	/**
	 * Perform the user friendship task (add or remove user friends).
	 */
	public UserConfig userFriendship(UserFriendsConfig config) {
		config.addCredentials(this);
		String xml = POST(this.url + "/user-friendship", config.toXML());
		UserConfig friend = null;
		if(xml == null || xml.equals("")) return friend;
		friend = new UserConfig();
		Element root = parse(xml);
		friend.parseXML(root);
		return friend;
	}
	 ...
	 */
	public List<UserConfig> getFriends(UserFriendsConfig config) {
		config.addCredentials(this);
		String xml = POST(this.url + "/get-user-friendships", config.toXML());
		List<UserConfig> friendsList = new ArrayList<>();
		Element root = parse(xml);
		if (root == null) {
			return friendsList;
		}
		for (int index = 0; index < root.getChildNodes().getLength(); index++) {
			UserConfig friend = new UserConfig();
			friend.parseXML((Element)root.getChildNodes().item(index));
	...
	/**
	 * Return all public user's profile.
	 */
	public List<UserConfig> getAllUsers(BrowseUserConfig config) {
		config.addCredentials(this);
		String xml = POST(this.url + "/get-all-users", config.toXML());
		List<UserConfig> publicUsersList = new ArrayList<UserConfig>();
		Element root = parse(xml);
		if (root == null) {
		...
		for (int index = 0; index < root.getChildNodes().getLength(); index++) {
			UserConfig user = new UserConfig();
			user.parseXML((Element)root.getChildNodes().item(index));
			publicUsersList.add(user);
		}
		return publicUsersList;
	}

	/**
	 * Return all user that have the given user as a friend.
	 */
	public List<UserConfig> getFollowers(UserFriendsConfig config) {
		config.addCredentials(this);
		String xml = POST(this.url + "/get-user-followers", config.toXML());
	...
	/**
	 * Return user bot greeting message.
	 */
	public UserMessageConfig getUserGreeting(UserMessageConfig config) {
		config.addCredentials(this);
		UserMessageConfig userMessageConfig = null;
		String xml = POST(this.url + "/get-user-greeting", config.toXML());
		if (xml == null || xml.equals("")) return userMessageConfig;
		userMessageConfig = new UserMessageConfig();
		Element root = parse(xml);
		userMessageConfig.parseXML(root);
	 ...
	 */
	public ArrayList<UserMessageConfig> getAllUserMessages(UserMessageConfig config) {
		config.addCredentials(this);
		String xml = POST(this.url + "/get-all-user-messages", config.toXML());
		ArrayList<UserMessageConfig> messagesList = new ArrayList<UserMessageConfig>();
		Element root = parse(xml);
		if (root == null) {
			return messagesList;
		}
		for (int index = 0; index < root.getChildNodes().getLength(); index++) {
			UserMessageConfig message = new UserMessageConfig();
			message.parseXML((Element)root.getChildNodes().item(index));
	...
	/**
	 * Return all user's friends info for current page.
	 */
	public ArrayList<UserMessageConfig> getUserConversations(UserMessageConfig config) {
		config.addCredentials(this);
		String xml = POST(this.url + "/get-user-conversations", config.toXML());
		ArrayList<UserMessageConfig> messagesList = new ArrayList<UserMessageConfig>();
		Element root = parse(xml);
		if(root == null) return messagesList;
		for (int index = 0; index < root.getChildNodes().getLength(); index++) {
			UserMessageConfig message = new UserMessageConfig();
			message.parseXML((Element)root.getChildNodes().item(index));
			messagesList.add(message);
	...
	/**
	 * Return all user to user messages.
	 */
	public ArrayList<UserMessageConfig> getUserToUserMessages(UserMessageConfig config) {
		config.addCredentials(this);
		String xml = POST(this.url + "/get-user-to-user-messages", config.toXML());
		ArrayList<UserMessageConfig> messagesList = new ArrayList<UserMessageConfig>();
		Element root = parse(xml);
		if (root == null) {
			return messagesList;
	...
	/**
	 * Return all user to user new messages.
	 */
	public ArrayList<UserMessageConfig> checkUserNewMessages(UserMessageConfig config) {
		config.addCredentials(this);
		String xml = POST(this.url + "/check-user-new-messages", config.toXML());
		ArrayList<UserMessageConfig> messagesList = new ArrayList<UserMessageConfig>();
		Element root = parse(xml);
		if (root == null) {
			return messagesList;
	...
	/**
	 * Deletes user conversation.
	 */
	public UserMessageConfig deleteUserConversation(UserMessageConfig config) {
		config.addCredentials(this);
		String xml = POST(this.url + "/delete-user-conversation", config.toXML());
		Element root = parse(xml);
		if (root == null) {
			return null;
		}
	...
	/**
	 * Return the user's voice configuration.
	 */
	public VoiceConfig getUserVoice(UserConfig config) {
		config.addCredentials(this);
		String xml = POST(this.url + "/get-user-voice", config.toXML());
		Element root = parse(xml);
		if (root == null) {
			return null;
		}
