...
public class UserVoiceActivity extends AbstractVoiceActivity implements TextToSpeech.OnInitListener, AdapterView.OnItemSelectedListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        HttpGetVoiceAction action = new HttpGetVoiceAction(this, userConfig);
        action.execute();
    }
    ...
    public void resetView(){
        try {
            ...
            speechSeekBar.setOnSeekBarChangeListener(
                    new SeekBar.OnSeekBarChangeListener() {
                        ...
                        public void onStopTrackingTouch(SeekBar seekBar) {
                            ...
                            Toast.makeText(UserVoiceActivity.this, getApplicationContext().getString(R.string.speechRate) + " " + String.valueOf(speechProgressValue), Toast.LENGTH_SHORT).show();
                        }
                    }
            );
            pitchSeekBar.setOnSeekBarChangeListener(
                    new SeekBar.OnSeekBarChangeListener() {
                        ...
                        public void onStopTrackingTouch(SeekBar seekBar) {
                            ...
                            Toast.makeText(UserVoiceActivity.this, getApplicationContext().getString(R.string.pitch) + " " + String.valueOf(pitchProgressValue), Toast.LENGTH_SHORT).show();
                        }
                    }
            );
            ...
            int index = Arrays.asList(MainActivity.voices).indexOf(MainActivity.user.voice);
            if (index != -1) {
            ...
            index = Arrays.asList(MainActivity.voiceMods).indexOf(MainActivity.user.voiceMod);
            if (index != -1) {
                voiceSpinMod.setSelection(index);
            }

            if(!"".equals(MainActivity.user.speechRate)) {
                speechProgressValue = Float.valueOf(MainActivity.user.speechRate);
            } else {
                speechProgressValue = 1.0f;
            }
            ...
            if(!"".equals(MainActivity.user.pitch)) {
                pitchProgressValue = Float.valueOf(MainActivity.user.pitch);
            } else {
                pitchProgressValue = 1.0f;
            }
