...
public class LiveChatConnection {
	...
	public void connect(ChannelConfig channel, UserConfig user) {
		...
		try {
			...
			this.socket.connect("wss://" + this.credentials.host + this.credentials.app
						+ "/live/chat", new WebSocketConnectionHandler() {
				@Override
				public void onOpen() {
					if (LiveChatConnection.this.user == null) {
						LiveChatConnection.this.socket.sendMessage("connect " + LiveChatConnection.this.channel.id + " " + LiveChatConnection.this.credentials.applicationId);
					} else {
						LiveChatConnection.this.socket.sendMessage(
	...
	public void accept() {
		checkSocket();
		this.socket.sendMessage("accept");
	}
	...
	public void pvt(String user) {
		checkSocket();
		this.socket.sendMessage("pvt: " + user);
	}
	...
	public void boot(String user) {
		checkSocket();
		this.socket.sendMessage("boot: " + user);
	}
	...
	public void whisper(String user, String message) {
		checkSocket();
		this.socket.sendMessage("whisper:" + user + ": " + message);
	}
