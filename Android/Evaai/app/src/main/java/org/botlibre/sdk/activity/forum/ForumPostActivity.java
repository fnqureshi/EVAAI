...
public class ForumPostActivity extends LibreActivity {
	...
	public void resetMenu() {
        ...
        if (instance.parent == null || instance.parent.length() == 0) {
        	menu.findItem(R.id.menuViewParent).setVisible(false);
        }
        if (instance.replies == null || instance.replies.isEmpty()) {
        	menu.findItem(R.id.menuViewReply).setVisible(false);
        }
        int position = MainActivity.posts.indexOf(this.instance);
        if (position < 0) {
        	menu.findItem(R.id.menuViewNext).setVisible(false);
        	menu.findItem(R.id.menuViewPrevious).setVisible(false);
        } else if (position == 0) {
        	menu.findItem(R.id.menuViewNext).setVisible(false);
        }
