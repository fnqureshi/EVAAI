...
/**
 * Commands allow chat messages to trigger actions on the device.
 * This is used for personal assistant commands and performing other actions.
 */
...
public class Command {
	...
	public Command(Context context, CommandProcessor processor, JSONObject jsonObject) {
	...
	public void audio() {
		String action = (String) jsonObject.opt("action");
		String src = (String) jsonObject.opt("src");
		Object loop = jsonObject.opt("loop");
		if (action != null) {
			if (action.equals("play")) {
				this.processor.playAudio(src, "true".equals(String.valueOf(loop)));
			}
		}
	}
	...
	public void timer() {
		String action = (String) jsonObject.opt("action");
		Object seconds = jsonObject.opt("millis");
		String postback = (String) jsonObject.opt("postback");
		long time = 1000;
		if (seconds != null) {
			try {
				time = Long.parseLong(String.valueOf(seconds));
			} catch (Exception exception) {
				// Ignore.
			}
		}
