	...
	public void resetView() {
		...
		forkAccModSpin.setSelection(Arrays.asList(MainActivity.forkAccMode).indexOf(instance.forkAccessMode));
			...
			protected void onActivityResult(int requestCode, int resultCode, Intent data) {
				...
				case 3:
					if(resultCode==RESULT_OK){
						String input = tagsText.getText().toString().trim();
						if(input.equals("") || input.endsWith(",")){
							tagsText.setText(input+data.getExtras().getString("tag") + ", ");
						} else {
							tagsText.setText(input+","+data.getExtras().getString("tag") + ", ");
						}
					}
				...
				case 4:
					if(resultCode==RESULT_OK){
						String input = categoriesText.getText().toString().trim();
						if(input.equals("") || input.endsWith(",")){
							categoriesText.setText(input + data.getExtras().getString("cat") + ", ");
						} else {
							categoriesText.setText(input+","+data.getExtras().getString("cat") + ", ");
						}
					}
